...
if __name__ == "__main__":
    ...
    # Load both cross-training and cross-testing dataset lists
    cross_train_dataset_list = cfg.cross_train_datasets if hasattr(cfg, 'cross_train_datasets') else [cfg.dataset]
    cross_test_dataset_list = cfg.cross_test_datasets if hasattr(cfg, 'cross_test_datasets') else cross_train_dataset_list

    if len(cross_train_dataset_list) != len(cross_test_dataset_list):
        raise ValueError("Error: The number of cross_train_datasets and cross_test_datasets must be equal to pair them up.")
    ...
    # --- OUTER LOOP: Iterate over paired train and test datasets ---
    for num_dataset, (train_dataset_name, test_dataset_name) in enumerate(zip(cross_train_dataset_list, cross_test_dataset_list)):
        # Generate the alias to find where the TRAINING weights were saved
        dataset_alias = f"dataset-{num_dataset + 1}"

        print(f"\n{'#' * 80}")
        print(f"### EVALUATING MISMATCH: Train Weights ({train_dataset_name}) on Test Data ({test_dataset_name}) ###")
        print(f"{'#' * 80}")

        # Data paths use the TEST dataset name
        params_csv_path = os.path.join(root_dir, "params_csv", test_dataset_name, "nebula_test_params.csv")
        numpy_path = os.path.join(root_dir, "cube", test_dataset_name, "test")

        # Dictionaries to accumulate metrics across seeds for this pairing
        seed_results = {
            'MAE': {'a': [], 'h': [], 'w': []},
            'RMSE': {'a': [], 'h': [], 'w': []}
        }
        ...
        for seed in seeds:
            ...
            # Load weights from the ALIAS directory (which represents the training data)
        ...
        if len(seed_results['MAE']['a']) > 0:
            # Construct the row for the CSV using BOTH names
            row_data = {
    ...
    if csv_data:
        ...
        # Save to the root of the save directory with a new custom name
        csv_save_path = os.path.join(save_root, f"{cfg.model}_train_test_diff_results.csv")
        df.to_csv(csv_save_path, index=False, encoding='utf-8-sig')

        print(f"\n{'=' * 80}")
        print(f"EVALUATION COMPLETE. Mismatch Results saved to:")
        print(f"{csv_save_path}")
        print(f"{'=' * 80}")
        print("\nSummary of Mean \u00B1 Std (MAE):")
        print(df[['Train_Dataset', 'Test_Dataset', 'MAE_a_Mean_Std', 'MAE_h_Mean_Std', 'MAE_w_Mean_Std']].to_string(
            index=False))
    else:
